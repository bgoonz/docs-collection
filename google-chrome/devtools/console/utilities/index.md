...
For more information on breakpoints, see [Pause Your Code With Breakpoints](/docs/devtools/javascript/breakpoints).
...
For more information, see the [`console.dir()`](/docs/devtools/console/api/#dir) entry in the Console API.

dirxml(object) {: \#dirxml-function }
-------------------------------------

`dirxml(object)` prints an XML representation of the specified object, as seen in the Elements tab. This method is equivalent to the [`console.dirxml()`](/docs/devtools/console/api/#dirxml) method.
...
`profile()` starts a JavaScript CPU profiling session with an optional name. `profileEnd()` completes the profile and displays the results in the Profile panel. (See also [Speed Up JavaScript Execution](/docs/devtools/rendering-tools/js-execution).)
