...
Prior to creating an audio configuration resource, the module should call `RecommendSampleFrameCount` to obtain a _sample frame count_ from the browser. The sample frame count is the number of samples that the callback function must provide per channel each time the browser invokes the callback function. For example, if the sample frame count is 4096 for a stereo audio stream, the callback function must provide a 8192 samples (4096 for the left channel and 4096 for the right channel).
