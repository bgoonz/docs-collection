...
Update `doExportToDisk()` with [`chrome.fileSystem.chooseEntry()`](/apps/fileSystem#method-chooseEntry) to allow the user to choose a file:
...
[`chrome.fileSystem.getDisplayPath()`](/apps/fileSystem#method-getDisplayPath) gets a displayable file path that outputs to the status `div`.
...
**Advanced**: `FileEntry` objects cannot be persisted indefinitely. Your app needs to ask the user to choose a file every time the app is launched. If your app was forced to restart due to a runtime crash or update, [restoreEntry()](/apps/fileSystem#method-restoreEntry) is an option to restore a `FileEntry`.
