...
{{EmbedLiveSample("Multi-line\_text\_fields", 120, 120)}}
...
`<textarea>` also takes a closing tag, and any default text you want it to contain should be put between the opening and closing tags. In contrast, the {{HTMLElement("input")}} is an empty element with no closing tag — any default value is put inside the `value` attribute.
...
On the {{HTMLElement("optgroup")}} element, the value of the `label` attribute is displayed before the values of the nested options. The browser usually sets them visually apart from the options (i.e. by being bolded and at a different nesting level) so they are less likely to be confused for actual options.
...
{{EmbedLiveSample("Multiple\_choice\_select\_box", 120, 120)}}
...
{{EmbedLiveSample("first\_autocomplete", 120, 120)}}
...
{{EmbedLiveSample("Datalist\_support\_and\_fallbacks", 120, 120)}}
