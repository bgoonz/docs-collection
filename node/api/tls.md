...
-   `options` {Object}
    ...
    -   `secureContext`: TLS context object created with [`tls.createSecureContext()`](#tls_tls_createsecurecontext_options). If a `secureContext` is *not* provided, one will be created by passing the entire `options` object to `tls.createSecureContext()`.
