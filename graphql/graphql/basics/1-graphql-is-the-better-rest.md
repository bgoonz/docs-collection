...
![Data Fetching with REST](https://imgur.com/VRyV7Jh.png) *With REST, you have to make three requests to different endpoints to fetch the required data. You’re also *overfetching* since the endpoints return additional information that’s not needed.*
...
![Data Fetching with GraphQL](https://imgur.com/z9VKnHs.png) *Using GraphQL, the client can specify exactly the data it needs in a *query*. Notice that the *structure* of the server’s response follows precisely the nested structure defined in the query.*
